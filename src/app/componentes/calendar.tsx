...
if (leftCaret) {
	leftCaret.addEventListener("click", nextMonth);
}
const rightCaret = document.querySelector(".fa-caret-right");
if (rightCaret) {
	rightCaret.addEventListener("click", prevMonth);
}
...
var currentColor: { value: any; name: any; id?: number; };
...
var notes: { id: number; title: any; desc: any; date: string; }[];
let staticNotes = [
	{
		id: 235684345,
...
function applyTheme() {
	...
	if (tableElement) {
		tableElement.style.boxShadow = `0px 0px 149px -28px ${currentColor.value}`;
	}

	const colorElement = document.querySelector(".color");
	if (colorElement) {
		(colorElement as HTMLElement).style.backgroundColor = `${currentColor.value}`;
	}

	const borderColorElement = document.querySelector(".border-color");
	if (borderColorElement) {
		(borderColorElement as HTMLElement).style.backgroundColor = `${currentColor.value}`;
	}

	for (let i = 0; i < 7; i++) {
		(document.querySelectorAll(".weekday")[
			i
		] as HTMLElement).style.backgroundColor = `${currentColor.value}`;
	}
}

if (currentDayDOM) {
	currentDayDOM.innerHTML = daysStr[todayDay as keyof typeof daysStr];
}
if (currentDateDOM) {
	currentDateDOM.innerHTML = todayDate.toString();
}
if (currentMonthDOM) {
	currentMonthDOM.innerHTML = monthsStr[state.todayMonth as keyof typeof monthsStr];
}
if (currentYearDOM) {
	currentYearDOM.innerHTML = todayYear.toString();
}
var currentFullYear = analyizYear(state.todayYear);
var currentFullMonth = currentFullYear.months[monthsStr[state.todayMonth as keyof typeof monthsStr] as keyof typeof currentFullYear.months];

//run App
showCalenderInfo();

//exp: analyizYear(2019) will get you all months length,first day,last day with indexes
function analyizYear(year: number) {
	let counter = 0;
	const currentYear = {
		year: year,
		is_leap: false,
		months: {
			Jan: {} as ReturnType<typeof analyizMonth>,
			Feb: {} as ReturnType<typeof analyizMonth>,
			Mar: {} as ReturnType<typeof analyizMonth>,
			Apr: {} as ReturnType<typeof analyizMonth>,
			May: {} as ReturnType<typeof analyizMonth>,
			Jun: {} as ReturnType<typeof analyizMonth>,
			Jul: {} as ReturnType<typeof analyizMonth>,
			Aug: {} as ReturnType<typeof analyizMonth>,
			Sep: {} as ReturnType<typeof analyizMonth>,
			Oct: {} as ReturnType<typeof analyizMonth>,
			Nov: {} as ReturnType<typeof analyizMonth>,
			Dec: {} as ReturnType<typeof analyizMonth>
		}
	};

	while (counter < 12) {
		Object.keys(currentYear.months).forEach(month => {
			currentYear.months[month as keyof typeof currentYear.months] = analyizMonth(month, year);
		});
		counter++;
	}
...
function analyizMonth(month: string, year: number) {
	const testDays = 31;
	let counter = 0;
	...
	const monthObj = {
		year: year,
		month: month,
		month_idx: monthsIndex[month as keyof typeof monthsIndex],
		first_day: "",
		first_day_index: null as number | null,
		days_length: 0,
		last_day_index: null as number | null,
		last_day: ""
	};
	...
	while (counter < testDays) {
		...
		if (dateArr[1] === month) {
			if (counter === 1) {
				...
				monthObj.first_day_index = daysIndex[dateArr[0] as keyof typeof daysIndex];
			}
			monthObj.last_day = dateArr[0];
			monthObj.last_day_index = daysIndex[dateArr[0] as keyof typeof daysIndex];
			monthObj.last_day_index = daysIndex[dateArr[0] as keyof typeof daysIndex];
		} else {
			return monthObj;
		}
...
function makePrevMonthArr(firstDayIndex: number | null) {
	let prevMonthIdx;
	let prevMonthDays;
	if (state.todayMonth === 0) {
		prevMonthDays = analyizMonth("Dec", state.todayYear - 1).days_length;
	} else {
		prevMonthIdx = monthsIndex[currentFullMonth.month as keyof typeof monthsIndex] - 1;
		prevMonthDays = currentFullYear.months[monthsStr[prevMonthIdx as keyof typeof monthsStr] as keyof typeof currentFullYear.months].days_length;
	}
	let result = [];
...
function printMonthCalendarInDOM() {
	...
	for (let i = 0; i < 6; i++) {
		...
		//
		const week = document.querySelector("#table-body")?.children[i] as HTMLElement;
		if (week) {
			for (let j = 0; j < 7; j++) {
				const dayElement = week.children[j] as HTMLElement;
			...
			if (
				...
				(week.children[j] as HTMLElement).style.backgroundColor = "#e1e1e1";
				currentMonthStarted = false;
				currentMonthEnd = false;
			} else {
				(week.children[j] as HTMLElement).style.cursor = "";
				(week.children[j] as HTMLElement).style.backgroundColor = "white"; //.style.backgroundColor = "white";
				(week.children[j] as HTMLElement).style.color = "black";
				week.children[j].innerHTML = currentWeek[j].day;
				week.children[j].removeAttribute("id");
				if (currentWeek[j].state !== "currMonth") {
					(week.children[j] as HTMLElement).style.backgroundColor = currentColor.value;
					(week.children[j] as HTMLElement).style.opacity = "0.6";
					(week.children[j] as HTMLElement).style.color = "rgba(255, 255, 255,0.4)";
					(week.children[j] as HTMLElement).style.cursor = "default";
